  ...
  */
  ...
  public:
    bool doTest(const Scanner::ITokCollection* pTc) override
    {
      ...
      if (tc.length() > 0 && tc[0] == "using" && tc[2]=="=")
      {
        doActions(pTc);
        return IRule::Stop;
      }

	  if (tc.length() > 0 && tc[0] == "typedef" && !tc[tc.length() - 2].empty())
	  {
		  doActions(pTc);
		  return IRule::Stop;
	  }
  ...
  public:
    ...
    void doAction(const Scanner::ITokCollection* pTc) override
    {
      ...
      if (se[0] == "using" && se[2]=="=")
      {
		...
		pCurrNode->name_ = se[1];
		pCurrNode->type_ = "alias";
		pCurrNode->path_ = p_Repos->currentPath();
		pCurrNode->package_ = declNode.package_;

        GrammarHelper::showParse("using declar", *pTc);
        return;
      }

	  if (se[0] == "typedef" && !se[se.length()-2].empty())
	  {
		  pCurrNode->name_ = se[se.length() - 2];
		  pCurrNode->type_ = "typedef";
		  pCurrNode->package_ = declNode.package_;
