...
// ver 1.3                                                         //
//-----------------------------------------------------------------//
// Yaodong Wang ,  derived from Jim Fawcett (c) copyright 2016     //
...
//              Yaodong Wang, CSE,  Syracuse University            //
/////////////////////////////////////////////////////////////////////
/*
  Note for project 2:
  The dependence analysis part is list at the last of the main function.
  The test case of cse667 project-2 demonstration requires specificly include
  following files: 
   - TypeAnal.h   :    this is where type table is created and used
   - DepAnal.h    :    where a specific file is tokenized and matched with TypeTable
                       Finally, the result is vent to NoSqlDb.
   - StrongComponent:  strong component evaluated, the file and its depdent subfiles 
					    will be loaded here to graph class defined here.
*/
#include "Executive.h"
#include <sstream>
#include <string>
...
namespace CodeAnalysis
{
	...
	*/
	...
	//----< show processing cpp implimentation >-------------------------------
	...
	//----< show processing c# >-------------------------------
	void CodeAnalysisExecutive::processCSharp(bool showProc)
	{
...
{
public:
	ExeInit(CodeAnalysisExecutive& exec, int argc, char* argv[])
	{
...
/////////////////////////////////////////////////////////////////////////////
/*
Test cases for DepAnal project
*/

class DepAnalHelper
{
public:
	DepAnalHelper() : ta(new TypeAnal()), dbshell(new Shell<string>("test"))
	{
	}

	// ----< test case for req1 2 >----
	void Req1_2()
	{
		Utils::Title("\n\nmeet Req1 using VC++ solution");
		Utils::Title("\nmeet Req2 using std");
	}

	// ----< test case for req3, list the package path... >----
	void Req3()
	{
		Utils::Title("\nReq3 ,  list my packages");
		string fileSpec = FileSystem::Path::getFullFileSpec("..\\CodeAnalyzer\\TypeAnal.h");
		Utils::title("  Type-Table and type analysis package :  " + fileSpec);
		fileSpec = FileSystem::Path::getFullFileSpec("..\\CodeAnalyzer\\DepAnal.h");
		Utils::title("  Dependency analysis package :  " + fileSpec);
		fileSpec = FileSystem::Path::getFullFileSpec("..\\StrongComponent\\StrongComponent.h");
		Utils::title("  Strong Component package :  " + fileSpec);
		fileSpec = FileSystem::Path::getFullFileSpec("..\\NoSqlDB");
		Utils::title("  NoSqlDB project  :   " + fileSpec);
		fileSpec = FileSystem::Path::getFullFileSpec("..") + "\\nosqldb.xml";
		Utils::title("  NoSqlDB   xml file:  " + fileSpec);
	}

	// ----< test case for req4 type table >----
	void Req4()
	{
		Utils::Title("\nReq4 - package TypeAnal.h demo");
	...
	// ----< test case for req5 dependency analysis >----
	void Req5(CodeAnalysisExecutive& exec)
	{
		Utils::Title("Req5 - package DepAnal.h demo");
		Utils::title("   Depedent Analysis in the file collection");
		da = new DepAnal(dbshell, ta->GetTable());
		for (auto map : exec.getFileMap())
			for (auto path : map.second)
			{
				string package = path.substr(path.find_last_of("\\") + 1);
				cout << "\n  analyzing  dependency of file: \"" << package << "\"\n";
				da->doTypeMatch(package, path);
			}
		da->doDbPersist();
		cout << da->demoDepAnal() << endl;
	}

	// ----< 6 StrongComponent :  strong component evaluated, the file and its depdent files loaded into graph, then run Tarjan alg >----
	void Req6()
	{
		Utils::Title("Req6 - Strong Connection Component ( SCC )");
		Utils::title("   list all SCC "); cout << endl;
		Graph g;
		vector<Vertex> vv;  // initial and stored vertex with static counter++ 's id;
		vector<string> keys = dbshell->getDB().getKeys();
		for (auto file : keys)
		{
			vv.push_back(Vertex(file));
			g.AddVertex(vv.back());
		}
		for (auto file : keys)
		{
			vector<Vertex>::iterator v, w;
			for (auto child : dbshell->queryChildren(file))
			{
				for (auto itor = begin(vv); itor != end(vv); ++itor)
					if (child == itor->GetValue())
					{
						w = itor; break;
					}
				for (auto itor = begin(vv); itor != end(vv); ++itor)
					if (file == itor->GetValue())
					{
						v = itor; break;
					}
				g.AddEdge(*v, *w);
		...
		StrongComponent sc; sc.Tarjan(g);
	}

	// ----< test case for req7, nosqldb.xml >----
	void Req7()
	{
		Utils::Title("Req7 - demo XML file from NoSqlDB");
		string fileSpec = FileSystem::Path::getFullFileSpec("..\\nosqldb.xml");
		Utils::title("  persist analysis result to  " + fileSpec);
		Utils::title(" \n      show the content of database             ");
		cout << dbshell->getDB().persistToXml() << endl;
	}

	// ----< test case for req8 >----
	void Req8(CodeAnalysisExecutive& exec, int argc, char* argv[])
	{
...
int main(int argc, char* argv[])
{
	...
	try {
		...
		ExeInit FawcettStructre(exec, argc, argv);
		DepAnalHelper dah;
		dah.Req1_2();
		dah.Req3();
		dah.Req4();
